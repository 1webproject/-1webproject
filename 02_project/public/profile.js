(function () {
    ...
    function createFollowingItem(user) {
        ...
        unfollowButton.addEventListener("click", function () {
            ...
            if (!result.success) {
                notify(result.message || "Could not update follow status.");
                return;
            }
    ...
    function setupPostCreation(viewedUser, currentUser) {
        ...
        postBtn.onclick = function () {
            ...
            if (!result.success) {
                notify(result.message || "Could not create post.");
                return;
            }
    ...
    function setupProfileActions(viewedUser, currentUser) {
        ...
        if (currentUser.id === viewedUser.id) {
            ...
            editBtn.onclick = function () {
                if (!window.AppUI || typeof window.AppUI.openProfileEditor !== "function") {
                    notify("Profile editor is unavailable right now.");
                    return;
                }
                window.AppUI.openProfileEditor(viewedUser, function (payload) {
                    const result = DataManager.updateUser(viewedUser.id, {
                        bio: payload.bio,
                        profilePicture: payload.profilePicture || viewedUser.profilePicture
                    });

                    if (!result.success) {
                        notify(result.message || "Could not update profile.");
                        return;
                    }
