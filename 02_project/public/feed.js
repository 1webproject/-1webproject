(function () {
  ...
  function attachPostActions() {
    ...
    likeButtons.forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        if (!result.success) {
          notify(result.message || "Could not update like.");
          return;
        }
  ...
  function attachFollowActions() {
    ...
    followButtons.forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        if (!result.success) {
          notify(result.message || "Could not follow this user.");
          return;
        }
  ...
  function renderSuggestedUsers() {
    ...
    const suggestedUsers = query && typeof DataManager.searchUsers === "function"
      ? DataManager.searchUsers(query, 8)
      : getSuggestedUsers();
  ...
  function setupCreatePost() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      reader.onload = function (event) {
        createPostWithImage(event.target.result);
      };
